...
class DriveService:
        ...
        if file_id:
            try:
                ...
                if not any(p in self.allowed_folder_ids for p in parents):
                    raise PermissionError(f"Access to file {file_id} is not allowed (not in allowed folders).")
    ...
    def list_all_files(self, pageSize: int = 100) -> List[Dict[str, Any]]:
        ...
        results = self.service.files().list(
            pageSize=pageSize, fields="nextPageToken, files(id, name, mimeType)"
        ).execute()
        return results.get("files", [])
    ...
    def upload_file(self, local_path: str, folder_id: str, filename: Optional[str] = None) -> Dict[str, Any]:
        self._verify_access(parent_id=folder_id)
